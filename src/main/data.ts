...
const ready = (async () => {
  ...
  run(
    "CREATE TABLE IF NOT EXISTS subjects (id TEXT PRIMARY KEY, name TEXT NOT NULL, color TEXT NOT NULL, config TEXT NOT NULL)",
  );
  ...
  run(
    "CREATE TABLE IF NOT EXISTS students (id TEXT PRIMARY KEY, name TEXT NOT NULL, [group] TEXT NOT NULL)",
  );
  ...
  run(
    "CREATE TABLE IF NOT EXISTS operation_logs (id TEXT PRIMARY KEY, description TEXT NOT NULL, created INTEGER NOT NULL, type TEXT NOT NULL, changes TEXT NOT NULL, reverted BOOLEAN NOT NULL)",
  );
  run(
    "CREATE TABLE IF NOT EXISTS identities (id TEXT PRIMARY KEY, name TEXT NOT NULL, role TEXT NOT NULL)",
  );
  run(
    "CREATE TABLE IF NOT EXISTS components (id TEXT PRIMARY KEY, x INTEGER NOT NULL, y INTEGER NOT NULL, width INTEGER NOT NULL, height INTEGER NOT NULL, type TEXT NOT NULL, config TEXT NOT NULL)",
  );
...
const ensureAssignmentConfig = (
  value: AssignmentConfig | null | undefined,
): AssignmentConfig => {
  return value && typeof value === "object" ? value : ({} as AssignmentConfig);
};

const ensureSubjectConfig = (
  value: SubjectConfig | null | undefined,
): SubjectConfig => {
...
async function ensureAdminIdentity() {
  const admin = getRow<Identity>("SELECT * FROM identities WHERE role = ?", [
    "admin",
  ]);
  if (!admin) {
    const identity = {
    ...
    run("INSERT INTO identities (id, name, role) VALUES (?, ?, ?)", [
      identity.id,
      identity.name,
      identity.role,
    ]);
...
function insertAssignmentRecord(assignment: Assignment) {
  ...
  run(
    "INSERT INTO assignments (id, subject, created, deadline, estimated, spent, title, description, priority, config) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [
      normalized.id,
...
function updateAssignmentRecord(assignment: Assignment) {
  const normalized = normalizeAssignment(assignment);
  run(
    "UPDATE assignments SET subject = ?, created = ?, deadline = ?, estimated = ?, spent = ?, title = ?, description = ?, priority = ?, config = ? WHERE id = ?",
    [
      normalized.subject.id,
      normalized.created.getTime(),
      normalized.deadline.getTime(),
...
function fetchSubmissionsForStudent(student: Student) {
  ...
  return rows.map((row) =>
    mapSubmissionRow(row, fetchAssignment(row.assignment)),
  );
}
...
function insertSubmissionRecord(submission: Submission) {
  ...
  run(
    "INSERT INTO submissions (id, assignment, student, created, spent, feedback) VALUES (?, ?, ?, ?, ?, ?)",
    [
      submission.id,
...
function insertDayRecord(day: Date, assignment: Assignment, taken: number) {
  ...
  run(
    "INSERT INTO day_record (date, assignment, subject, taken) VALUES (?, ?, ?, ?)",
    [date, assignment.id, assignment.subject.id, taken],
  );
}
...
function cutoffAssignment(assignment: Assignment, noDetail = false) {
  ...
  if ("totalRequiredSubmissions" in assignment)
    delete assignment.totalRequiredSubmissions;
...
export function cutoffAllUselessInfoInOperationLogs() {
  ...
  for (const operation of operations) {
    ...
    run("UPDATE operation_logs SET changes = ? WHERE id = ?", [
      operation.changes,
      operation.id,
    ]);
  }
...
const updateProgressOp = addOperation<[number, string][]>({
  ...
  async apply(changes) {
    for (const [progress, id] of changes) {
      run("UPDATE assignments SET spent = spent + ? WHERE id = ?", [
        progress,
        id,
      ]);
    }
...
async function doOperation<T, R>(
  op: Operation<T, R>,
  changes: T,
  description: string,
) {
  ...
  const id = randomId();
  run(
    "INSERT INTO operation_logs (id, description, created, type, changes, reverted) VALUES (?, ?, ?, ?, ?, ?)",
    [
      id,
      description,
      Date.now(),
...
const op2func = <T, R>(op: Operation<T, R>) => {
  return (changes: T, description: string) =>
    doOperation(op, changes, description);
};

async function undoOperation(id: string) {
  await ready;
  const log = getRow<OperationLogRow>(
  ...
  run("UPDATE operation_logs SET reverted = ?, changes = ? WHERE id = ?", [
    1,
...
const data: DataAPI = {
  assignment: {
    ...
    async list(begin: Date | null = null, end: Date | null = null) {
      ...
      return await Promise.all(
        assignmentSelect(
          "WHERE assignments.deadline >= ? AND assignments.created <= ?",
  ...
  student: {
    ...
    async add(studentOrName: Student | string, group?: string) {
      ...
      const student =
        typeof studentOrName === "string"
          ...
          : studentOrName;
      run("INSERT INTO students (id, name, [group]) VALUES (?, ?, ?)", [
        student.id,
        student.name,
        student.group,
      ]);
    ...
    async update(student: Student) {
      await ready;
      run("UPDATE students SET name = ?, [group] = ? WHERE id = ?", [
        student.name,
        student.group,
        student.id,
      ]);
  ...
  subject: {
    ...
    async add(subject) {
      await ready;
      run(
        "INSERT INTO subjects (id, name, color, config) VALUES (?, ?, ?, ?)",
        [
          subject.id,
          subject.name,
          subject.color,
          JSON.stringify(subject.config),
        ],
      );
      emitChange();
    },
    async update(subject) {
      await ready;
      run("UPDATE subjects SET name = ?, color = ?, config = ? WHERE id = ?", [
        subject.name,
        subject.color,
        JSON.stringify(subject.config),
        subject.id,
  ...
  identity: {
    async get(id) {
      await ready;
      const row = getRow<Identity>("SELECT * FROM identities WHERE id = ?", [
        id,
      ]);
      if (!row) {
    ...
    async update(identity) {
      await ready;
      run("UPDATE identities SET name = ?, role = ? WHERE id = ?", [
        identity.name,
        identity.role,
        identity.id,
      ]);
  ...
  tag: {
    ...
    async add(tag) {
      await ready;
      run("INSERT INTO assignment_tags (id, name, color) VALUES (?, ?, ?)", [
        tag.id,
        tag.name,
        tag.color,
      ]);
    ...
    async update(tag) {
      await ready;
      run("UPDATE assignment_tags SET name = ?, color = ? WHERE id = ?", [
        tag.name,
        tag.color,
        tag.id,
      ]);
  ...
  progress: {
    ...
    async within(begin, end) {
      ...
      const progress = logs.map((log) => [
        log.created,
        JSON.parse(log.changes) as [number, string][],
      ]) as [Date, [number, string][]][];
      const result = progress.map(([date, progress]) => [
        date,
        progress.map(([spent, id]) => [spent, fetchAssignment(id)]),
      ]) as [Date, [number, Assignment][]][];
      return result;
    },
  },
  operation: {
  ...
  component: {
    ...
    async add(config) {
      await ready;
      run(
        "INSERT INTO components (id, x, y, width, height, type, config) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [
          config.id,
          config.x,
          config.y,
    ...
    async update(config) {
      await ready;
      run(
        "UPDATE components SET x = ?, y = ?, width = ?, height = ?, type = ?, config = ? WHERE id = ?",
        [
          config.x,
          config.y,
          config.width,
  ...
  day: {
    async get(begin, end) {
      ...
      const rows = allRows<DayRow>(
        "SELECT * FROM day_record WHERE date BETWEEN ? AND ? ORDER BY date",
        [beginKey, endKey],
      );
