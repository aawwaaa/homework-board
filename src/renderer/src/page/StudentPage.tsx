          ...
          <>
            <div className="title">
              <Tab
                ...
                set={setPage}
              />
              <button className="outline" onClick={() => submit()}>
                快速提交
              </button>
            </div>
            <div className="content">
              ...
              {page === "submit" && (
                <SubmissionCreate student={student} submit={submit} />
              )}
            </div>
          </>
        )}
...
export const StudentPage: React.FC<{ left: string }> = ({ left }) => {
  ...
  useEffect(() => {
    return window.data.onChanged(async () => {
      ...
      const student = (await window.data.student.list()).find(
  ...
  return (
    <div className="student-page">
      <div className="title">
        <Tab tabs={[["todo", "未完成"]]} initial={page} set={setPage} />
      </div>
      <div className="body">
        {student == null ? (
          ...
          <>
            {page === "todo" && (
              <TodoPage assignments={assignments} student={student} />
            )}
          </>
        )}
      </div>
