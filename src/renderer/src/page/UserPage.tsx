...
export const UserPage: React.FC<{ left: string }> = ({ left }) => {
    ...
    const pushStack = useCallback((name: string, element: React.ReactNode) => {
    ...
    const turnPage = useCallback((name: string) => {
        const [guard, element] = pages[name];
        if (!guard || !element) {
            return;
        }
        const elem = element({ identity: identity!, updatePage(name, element) {
    ...
    return <div className="user-page">
        <div className={page in pages? "sidebar pages": "sidebar pages minimized"}>
            {/* <h3>{page}</h3> */}
