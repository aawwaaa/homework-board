...
const useDragReorder = (
    ...
    const dragState = useRef<{ pointerId: number; currentIndex: number } | null>(null);
    ...
    const resolveTargetIndex = useCallback(
        (pointerY: number, currentIndex: number) => {
            ...
            const items = Array.from(container.querySelectorAll<HTMLElement>("[data-index]"));
    ...
    return { draggingIndex, handlePointerDown, handlePointerMove, handlePointerEnd };
};

export const ScheduleEdit = ({ schedule, setSchedule, set, className, onClose }: ScheduleEditProps) => {
    const setter = setSchedule ?? set;
    const listRef = useRef<HTMLDivElement | null>(null);
    const [entries, setEntries] = useState<ScheduleEntry[]>(() => schedule.entries ?? []);
    const [expanded, setExpanded] = useState<boolean[]>(() => (schedule.entries ?? []).map(() => false));

    const [importSource, setImportSource] = useState<React.ReactNode | null>(null);
    ...
    const updateEntry = (index: number, patch: Partial<ScheduleEntry>) => {
        const nextEntries = entries.map((entry, i) => (i === index ? { ...entry, ...patch } : entry));
        commitEntries(nextEntries);
    };
    ...
    const addEntry = (entry?: ScheduleEntry) => {
        const nextEntries = [...entries, entry ?? { duration: 5 * MINUTE_IN_MS, title: "", description: "" }];
    ...
    const { draggingIndex, handlePointerDown, handlePointerMove, handlePointerEnd } = useDragReorder(listRef, reorderEntry);

    const containerClass = ["schedule-edit", className].filter(Boolean).join(" ");

    return (
        <div className={containerClass}>
            <div className={"schedule-import-source" + (importSource ? "" : " hidden")}>
                {importSource}
            </div>
            <div className="schedule-edit-toolbar">
                <button onClick={onClose} className="flat">关闭</button>
                <button onClick={() => setImportSource(<AssignmentProcess add={addEntries} close={() => setImportSource(null)} />)} className="outline">从作业信息导入</button>
            </div>
            <div className="schedule-edit-body" ref={listRef}>
                ...
                {entries.map((entry, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={isDragging ? "schedule-entry dragging" : "schedule-entry"}
                            data-index={index}
                        >
                            <div className="schedule-entry-row">
                                ...
                                <div className="duration-field">
                                    <SwipeAdjustInput
                                        ...
                                        value={minutes}
                                        inputMode="numeric"
                                        onChange={(e) => updateEntry(index, { duration: clampMinutes(Number(e.target.value)) * MINUTE_IN_MS })}
                                        onSwipeAdjust={(steps) =>
                                            updateEntry(index, { duration: clampMinutes(minutes + steps * DURATION_STEP_MIN) * MINUTE_IN_MS })
                                        }
                                        swipePxPerStep={34}
                                    />
                                ...
                                />
                                <button type="button" className="flat" onClick={() => toggleExpanded(index)}>
                                    {isExpanded ? "收起" : "扩展"}
                                </button>
                                <button type="button" className="flat danger" onClick={() => removeEntry(index)}>
                                    移除
                                </button>
                            </div>
                            {isExpanded && (
                ...
                <button type="button" className="schedule-add primary" onClick={() => addEntry()}>
                    添加
                </button>
            </div>
        </div>
