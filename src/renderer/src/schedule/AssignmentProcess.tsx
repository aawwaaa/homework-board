...
const alignToUnit = (value: number, unit: number) =>
...
> = {
  优先级均分: (assignments, available, unit) => {
    ...
    for (const level of Object.entries(levels).sort((a, b) => +b[0] - +a[0])) {
      ...
      const comsumption = assignments.reduce(
        (a, b) => a + Math.max(0, targetFor(b) - b.spent),
        0,
      );
...
export const AssignmentProcess: React.FC<{
  add: (entries: ScheduleEntry[]) => void;
  close: () => void;
}> = ({ add, close }) => {
  ...
  const confirm = () => {
    ...
    window.data.progress.update(
      Object.entries(entries).map(([a, b]) => [b, a]),
      "从作业数据导入",
    );
    add(out);
    close();
  ...
  return (
    <div className="assignment-process">
      <div className="schedule-import-source-header">
        <button onClick={close} className="flat">
          关闭
        </button>
        <select value={solver} onChange={(e) => setSolver(e.target.value)}>
          {Object.entries(solvers).map(([name, _]) => (
        ...
        <SwipeAdjustInput
          type="number"
          value={available}
          onChange={(e) => setAvailable(Math.max(0, e.target.valueAsNumber))}
          swipePxPerStep={30}
          onSwipeAdjust={(steps) =>
            setAvailable(Math.max(0, available + steps * unit))
          }
          placeholder="可用时间"
        />
        <SwipeAdjustInput
          type="number"
          value={unit}
          onChange={(e) => setUnit(Math.max(5, e.target.valueAsNumber))}
          swipePxPerStep={30}
          onSwipeAdjust={(steps) => setUnit(Math.max(5, unit + steps * 5))}
          placeholder="时间单位"
        />
        <button onClick={solve} className="secondary">
          求解
        </button>
            ...
            <div key={assignment.id} className="assignment-process-item">
              ...
              >
                <div
                  ...
                  style={{
                    width:
                      (assignment.spent / assignment.estimated) * 100 + "%",
                  }}
                />
                <div
                  className="delta"
