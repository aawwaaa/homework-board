...
const overlapsAssignmentRange = (assignment: AssignmentData, interval: Interval) => {
...
const defaultAssignmentFilter: (assignment: AssignmentData) => boolean = () => true;
...
export const TimelineView: FC<{
    ...
    const [assignmentMap, setAssignmentMap] = useState<Map<string, AssignmentData>>(() => new Map());
    ...
    const touchPanState = useRef<{ identifier: number; startX: number; startY: number; startViewX: number; startViewY: number } | null>(null);
    const pinchState = useRef<{ startDistance: number; startScale: number } | null>(null);

    const unit = unitWidth ?? 1;
    const timeToPosition = useCallback((time: Date, originDate?: Date) => {
        const diff = time.getTime() - (originDate ?? originalOrigin).getTime();
        return diff / DAY * unit;
    }, [originalOrigin, unit]);
    const positionToTime = useCallback((position: number, originDate?: Date) => {
    ...
    const loadRange = useCallback(async (lower: Date, upper: Date, options?: { force?: boolean }) => {
        const normalized = normalizeInterval({ start: lower.getTime(), end: upper.getTime() });
        if (normalized.end <= normalized.start) {
            return;
        }
        const intervalsToFetch = options?.force ? [normalized] : getMissingIntervals(loadedIntervals.current, normalized);
        if (intervalsToFetch.length === 0) {
            return;
        }
        try {
            const responses = await Promise.all(intervalsToFetch.map(interval => window.data.assignment.list(new Date(interval.start), new Date(interval.end))));
            setAssignmentMap(prev => {
                const next = new Map(prev);
                let changed = false;
                if (options?.force) {
                    next.forEach((assignment, id) => {
                        if (intervalsToFetch.some(interval => overlapsAssignmentRange(assignment, interval))) {
                            next.delete(id);
                            changed = true;
                        }
                    });
            ...
            intervalsToFetch.forEach(interval => addTicks(new Date(interval.start), new Date(interval.end)));
            loadedIntervals.current = mergeIntervals([...loadedIntervals.current, normalized]);
        } catch (error) {
    ...
    useEffect(() => {
        const unsubscribe = window.data.onChanged(() => {
            ...
            loadedIntervals.current.forEach(interval => {
                void loadRange(new Date(interval.start), new Date(interval.end), { force: true });
            });
        });
        return unsubscribe;
    }, [loadRange]);
    ...
    const laneData = useMemo<LaneItem[][]>(() => {
        ...
        const sorted = Array.from(assignmentMap.values()).sort((a, b) => a.created.getTime() - b.created.getTime());
    ...
    const trackElements = useMemo<React.ReactNode[]>(() => {
        ...
        return laneData.map((lane, index) => {
            ...
            return <div className="track" key={`track-${index}`}>{children}</div>;
        });
    }, [laneData, builder]);
    ...
    const onTouchMove = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
        ...
        if (e.touches.length === 1) {
            ...
            if (!touchPanState.current || touchPanState.current.identifier !== touch.identifier) {
                startTouchPan(touch);
            }
    ...
    >
        <div style={timelineStyle}>
            {tickDates.map(timestamp => {
                ...
                return <div className="tick" key={timestamp} style={{ left: timeToPosition(date) }}>{date.toLocaleDateString()}</div>;
            })}
