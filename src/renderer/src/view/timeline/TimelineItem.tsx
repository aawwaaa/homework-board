...
export const StandardTimelineItem: FC<{
  assignment: AssignmentData;
  x: number;
  width: number;
  onClick?: () => void;
}> = ({ assignment, x, width, onClick }) => {
  ...
  return (
    <div className="timeline-item" style={style} key={assignment.id}>
      <div
        className={"timeline-item-bar" + " priority-" + assignment.priority}
        onClick={onClick}
        style={{ cursor: onClick ? "pointer" : "default" }}
      >
        <AssignmentTitle assignment={assignment} />
        <div
          className="timeline-item-time-progress"
          style={{ width: (width * assignment.spent) / assignment.estimated }}
        >
          ...
          <span>
            {" "}
            ({Math.round((assignment.spent / assignment.estimated) * 100)}%)
          </span>
        </div>
        <div
          className="timeline-item-submission-progress"
          style={{
            width:
              (width * assignment.submissions.length) /
...
export const standardTimelineItemBuilder = (
  assignment: AssignmentData,
