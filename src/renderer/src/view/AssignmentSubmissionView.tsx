...
const normalizeInterval = (interval: Interval): Interval => {
    return interval.start <= interval.end ? interval : { start: interval.end, end: interval.start };
};
...
const mergeIntervals = (intervals: Interval[]): Interval[] => {
    ...
    const sorted = intervals.map(normalizeInterval).sort((a, b) => a.start - b.start);
...
const getMissingIntervals = (existing: Interval[], target: Interval): Interval[] => {
    ...
    for (const interval of merged) {
        ...
        if (interval.start > cursor) {
            missing.push({ start: cursor, end: Math.min(interval.start, normalized.end) });
        }
...
export const AssignmentSubmissionView: FC<AssignmentSubmissionViewProps> = ({
    ...
    const [internalStart, setInternalStart] = useState<Date>(() => (start ? clampDate(start) : addDays(new Date(), -14)));
    const [internalEnd, setInternalEnd] = useState<Date>(() => (end ? clampDate(end) : new Date()));

    const effectiveStartMs = start ? start.getTime() : internalStart.getTime();
    const effectiveEndMs = end ? end.getTime() : internalEnd.getTime();
    const effectiveStart = useMemo(() => new Date(effectiveStartMs), [effectiveStartMs]);
    const effectiveEnd = useMemo(() => new Date(effectiveEndMs), [effectiveEndMs]);
    ...
    const [submissionMap, setSubmissionMap] = useState<Map<string, Submission>>(() => new Map());

    const loadRange = useCallback(
        async (lower: Date, upper: Date, options?: { force?: boolean }) => {
            const normalized = normalizeInterval({ start: lower.getTime(), end: upper.getTime() });
            if (normalized.end <= normalized.start) {
                return;
            }
            const intervalsToFetch = options?.force ? [normalized] : getMissingIntervals(loadedIntervalsRef.current, normalized);
            ...
            try {
                const responses = await Promise.all(
                    intervalsToFetch.map(async (interval) => {
                        ...
                        const [filteredAssignments, within] = await Promise.all([
                            typeof filter === "function" ? window.data.assignment.list(begin, end) : Promise.resolve(null),
                            window.data.submission.within(begin, end),
                        ]);
                        const allowed =
                            filteredAssignments && typeof filter === "function"
                                ? new Set(filteredAssignments.filter(filter).map((assignment) => assignment.id))
                                : null;
                        return allowed ? within.filter((submission) => allowed.has(submission.assignment.id)) : within;
                    }),
                );

                if (requestId === requestIdRef.current) {
                    setSubmissionMap((prev) => {
                        const next = options?.force ? new Map<string, Submission>() : new Map(prev);
                ...
                setPendingRequests((prev) => Math.max(0, prev - intervalsToFetch.length));
            }
        },
        [filter],
    );
    ...
    useEffect(() => {
        ...
        const next = { startMs: effectiveStart.getTime(), endMs: effectiveEnd.getTime() };
        const isExpanded =
            prev != null && next.startMs <= prev.startMs && next.endMs >= prev.endMs;
        const shouldForce = prev == null || !isExpanded;
        prevRangeRef.current = next;
        loadRange(effectiveStart, effectiveEnd, shouldForce ? { force: true } : undefined);
    ...
    const submissions = useMemo(() => Array.from(submissionMap.values()), [submissionMap]);
    const bins = useMemo(() => generateBins(effectiveStart, effectiveEnd), [effectiveEnd, effectiveStart]);

    const { maxTotal, detailsByBin, assignmentIndex, visibleCount, skippedCount } = useMemo(() => {
        const details = new Map<
            number,
            Map<string, { assignment: Assignment; submissionCount: number; students: Map<string, Student> }>
        >();
        const assignmentMap = new Map<string, { assignment: Assignment; color: string }>();
        ...
        submissions.forEach((submission) => {
            const created = submission.created instanceof Date ? submission.created : new Date(submission.created);
            ...
            const assignmentEntry =
                ...
                (() => {
                    const value = { assignment: submission.assignment, color: colorForAssignment(submission.assignment) };
            ...
            const byAssignment =
                details.get(binKey) ??
                (() => {
                    const value = new Map<string, { assignment: Assignment; submissionCount: number; students: Map<string, Student> }>();
            ...
            const entry =
                ...
                (() => {
                    const value = { assignment: assignmentEntry.assignment, submissionCount: 0, students: new Map<string, Student>() };
        ...
        const max = Array.from(countsByBin.values()).reduce((acc, value) => Math.max(acc, value), 0);
    ...
    const extendRange = useCallback(
        (direction: -1 | 1) => {
            const nextStart = direction === -1 ? addDays(effectiveStart, -extendDays) : effectiveStart;
            const nextEnd = direction === 1 ? addDays(effectiveEnd, extendDays) : effectiveEnd;
            if (direction === -1) {
                pendingViewXAdjustPxRef.current = countPrependedBins(nextStart, effectiveStart) * BIN_WIDTH_PX;
            }
    ...
    const touchPanState = useRef<{ identifier: number; startX: number; startViewX: number } | null>(null);
    ...
    const onTouchStart = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
        ...
        touchPanState.current = { identifier: touch.identifier, startX: touch.clientX, startViewX: viewXRef.current };
    }, []);
    ...
    const onTouchMove = useCallback(
        (event: React.TouchEvent<HTMLDivElement>) => {
            ...
            const touch = Array.from(event.touches).find((t) => t.identifier === state.identifier);
            if (!touch) {
                return;
            }
    ...
    const dayTicks = useMemo(() => {
        return bins
            ...
            .filter((value): value is { timestamp: number; left: number; label: string } => Boolean(value));
    }, [bins]);
    ...
    const [selectedCounts, setSelectedCounts] = useState<{ submissionCount: number; submitterCount: number }>({
        submissionCount: 0,
        submitterCount: 0,
    });
    ...
    useEffect(() => {
        ...
        setSelectedCounts({ submissionCount: entry.submissionCount, submitterCount: studentList.length });
    ...
    return (
        <div className="assignment-submission-view">
            <div className="asv-toolbar">
                <span className={["asv-status", error ? "error" : isLoading ? "loading" : ""].filter(Boolean).join(" ")}>
                    {statusText}
                </span>
                <div className="asv-actions">
                    <button type="button" className="outline" onClick={() => extendRange(-1)}>
                        向前 {extendDays} 天
                    </button>
                    <button type="button" className="outline" onClick={() => extendRange(1)}>
                        向后 {extendDays} 天
            ...
            >
                <div className="asv-track" style={trackStyle}>
                    {dayTicks.map((tick) => (
                        <div key={tick.timestamp} className="asv-day-tick" style={{ left: tick.left }}>
                            {tick.label}
                        </div>
                    ))}
                    {bins.map((binStart) => {
                        const byAssignment = detailsByBin.get(binStart);
                        const entries = byAssignment ? Array.from(byAssignment.entries()) : [];
                        const segments = entries
                            .map(([assignmentId, entry]) => {
                                const color = assignmentIndex.get(assignmentId)?.color ?? colorForAssignment(entry.assignment);
                                return { assignmentId, assignment: entry.assignment, color, count: entry.submissionCount };
                            })
                            .filter((segment) => segment.count > 0)
                            .sort(
                                (a, b) =>
                                    b.count - a.count ||
                                    a.assignment.title.localeCompare(b.assignment.title, "zh-CN") ||
                            ...
                            <div key={binStart} className="asv-bin">
                                <div className="asv-bin-bar">
                                    {segments.map((segment) => {
                                        const rawHeight =
                                            maxTotal === 0 ? 0 : Math.max(1, Math.round((segment.count / maxTotal) * chartHeightPx));
                                        const height = Math.max(MIN_SEGMENT_HEIGHT_PX, rawHeight - SEGMENT_GAP_PX);
                                        const style: CSSProperties = { backgroundColor: segment.color, height, bottom };
                                        bottom += height + SEGMENT_GAP_PX;
                                        const isSelected =
                                            selected?.binStart === binStart && selected.assignmentId === segment.assignmentId;
                                        return (
                                            <button
                                                key={segment.assignmentId}
                                                type="button"
                                                className={["asv-segment", isSelected ? "selected" : ""].filter(Boolean).join(" ")}
                                                style={style}
                                                title={`${segment.assignment.title}：${segment.count}`}
                                                aria-label={`${segment.assignment.title}：${segment.count}`}
                                                onClick={() => setSelected({ binStart, assignmentId: segment.assignmentId })}
                                            />
                                        );
                                    })}
                    ...
                    <div className="asv-detail-header">
                        <div className="asv-detail-title">
                            {selectedAssignmentData ? (
                                <AssignmentTitle assignment={selectedAssignmentData} classList="asv-assignment-title" />
                            ) : (
                                <strong className="asv-assignment-title-fallback">
                                    {assignmentIndex.get(selected.assignmentId)?.assignment.title ?? selected.assignmentId}
                                </strong>
                            )}
                            <span className="asv-detail-meta">
                                {selectedCounts.submissionCount} 次提交 · {selectedCounts.submitterCount} 人
                            </span>
                        </div>
                        <button type="button" className="flat" onClick={() => setSelected(null)}>
                            关闭
                        </button>
                    </div>
