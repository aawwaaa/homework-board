...
export const AssignmentList: React.FC<AssignmentListProps> = ({ onClick, className }) => {
    ...
    const visibleAssignments = useMemo(() => {
        const filtered = selectedSubject === ALL_SUBJECT
            ? assignments
            : assignments.filter(assignment => assignment.subject.id === selectedSubject);
        return filtered
            .slice()
            .sort((a, b) => a.deadline.getTime() - b.deadline.getTime() || a.created.getTime() - b.created.getTime());
    ...
    const containerClass = ["assignment-list", className].filter(Boolean).join(" ");
    ...
    const renderAssignment = (assignment: AssignmentData) => {
        ...
        return (
            <li
                key={assignment.id}
                className={clickable ? "assignment-list-item clickable" : "assignment-list-item"}
        ...
        <div className={containerClass}>
            ...
            <section className="assignment-list-content">
                <div className="assignment-list-meta">
                    ...
                    <span>{loading ? "加载中…" : `共 ${visibleAssignments.length} 个`}</span>
                </div>
                {loading ? (
