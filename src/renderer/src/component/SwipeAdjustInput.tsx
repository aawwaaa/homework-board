...
const useSwipeAdjust = <T extends HTMLElement>({
  ...
  const resetState = useCallback((target?: EventTarget & T) => {
    ...
    if (
      ...
      target?.hasPointerCapture?.(state.pointerId)
    ) {
...
export type SwipeAdjustInputProps = Omit<
...
export const SwipeAdjustInput = forwardRef<
  HTMLInputElement,
  SwipeAdjustInputProps
>((props, ref) => {
  const {
    type,
    className,
  ...
  const pointerHandlers = useMemo(
    () => ({
      ...
      onPointerLeave: chainHandlers(
        swipeHandlers.onPointerLeave,
        onPointerLeave,
      ),
      onPointerCancel: chainHandlers(
