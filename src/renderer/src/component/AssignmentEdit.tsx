...
const parseDateTimeLocal = (value: string) => {
  ...
  const parsed = [yearStr, monthStr, dayStr, hourStr, minuteStr].map((part) =>
    Number(part),
  );
...
const combineMinutesToMs = (hours: number, minutes: number) =>
  combineMinutes(hours, minutes) * MINUTE_IN_MS;
...
type AssignmentEditPresetProps = BaseProps & {
  ...
  onChange: <K extends keyof AssignmentPreset>(
    key: K,
    value: AssignmentPreset[K],
  ) => void;
};

type AssignmentEditProps =
  | AssignmentEditAssignmentProps
  | AssignmentEditPresetProps;

const isAssignmentProps = (
  props: AssignmentEditProps,
): props is AssignmentEditAssignmentProps => {
  return "subject" in props.value;
};

const isPresetProps = (
  props: AssignmentEditProps,
): props is AssignmentEditPresetProps => {
...
export const AssignmentEdit = (props: AssignmentEditProps) => {
  ...
  const containerClass = ["assignment-edit", className]
    .filter(Boolean)
    .join(" ");

  const titleValue = assignment
  ...
  const durationParts = preset
    ? splitDuration(preset.value.duration ?? 0)
    : null;

  const createdRef = useRef<Date | null>(
    assignment ? new Date(assignment.value.created) : null,
  );
  const deadlineRef = useRef<Date | null>(
    assignment ? new Date(assignment.value.deadline) : null,
  );
  ...
  useEffect(() => {
    return window.data.onChanged(async () => {
      setTags(
        Object.fromEntries(
          (await window.data.tag.list()).map((a) => [a.id, a]),
        ),
      );
    });
  }, []);
  ...
  const handleDateChange =
    (key: "created" | "deadline") => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleDurationChange =
    (part: "days" | "hours") => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleEstimatedChange =
    (part: "hours" | "minutes") => (event: ChangeEvent<HTMLInputElement>) => {
      if (disabled) {
        return;
      }
      const nextValue = toNumber(event.target.value);
      ...
      const nextMinutes =
        part === "minutes" ? nextValue : estimatedParts.minutes;
      if (assignment) {
        assignment.onChange(
          "estimated",
          combineMinutes(nextHours, nextMinutes),
        );
      } else if (preset) {
        preset.onChange(
          "estimated",
          combineMinutesToMs(nextHours, nextMinutes),
        );
      }
    };
    ...
    <div className={containerClass} aria-readonly={disabled}>
      <label className="assignment-field">
        ...
        <input
          ...
          disabled={disabled}
        />
        {assignment && assignment.presets ? (
          <select onChange={handlePresetChange} ref={presetSelectRef}>
        ...
        <div className="assignment-inline-group">
          ...
          <label className="assignment-inline-input">
            <SwipeAdjustInput
              ...
              disabled={disabled}
              onSwipeAdjust={(steps) =>
                adjustEstimatedBySwipe("minutes", steps)
              }
              swipeDisabled={disabled}
              swipePxPerStep={18}
            />
        ...
        <div className="assignment-field">
          <div className="assignment-inline-group">
            ...
            <SwipeAdjustInput
              ...
              onChange={(value) =>
                assignment.onChange("spent", +value.target.value)
              }
              disabled={disabled}
              onSwipeAdjust={(steps) =>
                assignment.onChange("spent", assignment.value.spent + steps * 5)
              }
              swipeDisabled={disabled}
            />
            <span>分钟</span>
