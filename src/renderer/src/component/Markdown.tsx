...
const renderEmphasis = (source: string, keyBase: string): ReactNode[] => {
  const renderItalic = (input: string, italicKeyBase: string) =>
    splitByRegex(
      input,
      /\*([^*]+)\*/g,
      (match, key) => <em key={key}>{match[1]}</em>,
...
const renderInlineWithLineBreaks = (
  source: string,
  keyBase: string,
): ReactNode[] => {
...
const parseBlocks = (text: string): Block[] => {
  ...
  while (index < lines.length) {
    ...
    if (colorBlock) {
      ...
      const colorSpec =
        sanitizeColor(rawSpec.split("(")[0]?.trim() ?? "") ?? "red";
  ...
  return blocks.map((block, index) => {
    ...
    if (block.type === "heading") {
      ...
      return createElement(
        tag,
        { key: keyBase },
        renderInline(block.text, `${keyBase}.h`),
      );
    }
    if (block.type === "code") {
      return (
        ...
        <ul key={keyBase}>
          {block.items.map((item, itemIndex) => (
            <li key={`${keyBase}.li${itemIndex}`}>
              {renderInlineWithLineBreaks(item, `${keyBase}.li${itemIndex}`)}
            </li>
          ))}
        </ul>
      );
    ...
    return (
      <p key={keyBase}>
        {renderInlineWithLineBreaks(block.text, `${keyBase}.p`)}
      </p>
    );
  });
};
