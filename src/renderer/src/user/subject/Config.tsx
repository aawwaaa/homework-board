...
export const SubjectConfigPage = ({ subject }: { subject: Subject }) => {
  ...
  const handleAddPreset = useCallback(() => {
    ...
    updateSubject((prev) => {
      const nextPresets = [
        ...(prev.config.assignmentPresets ?? []),
        createPreset(nextIndex),
      ];
      return {
        ...prev,
        config: {
  ...
  const updatePresetField = useCallback(
    (
      index: number,
      key: keyof AssignmentPreset,
      value: AssignmentPreset[keyof AssignmentPreset],
    ) => {
      updateSubject((prev) => {
        ...
        {!presetTabs.length ? (
          ...
          <>
            <AssignmentEdit
              ...
              value={activePreset}
              onChange={(key, value) =>
                updatePresetField(activePresetIndex, key, value)
              }
            />
