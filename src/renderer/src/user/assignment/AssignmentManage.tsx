...
export const AssignmentManage: React.FC<{
  ...
  const confirmEdit = () => {
    window.data.assignment.modify(
      editingAssignment,
      `[${editingAssignment.subject.name}] 修改作业 ${editingAssignment.title}`,
    );
    props.popPage();
  };
  const handleChange = (
  ...
  return (
    <div className="assignment-manage">
      <div className="title">
        <Tab
          tabs={[
            ["edit", "编辑"],
        ...
        <>
          ...
          <div>
            ...
            <button
              className="danger"
              onClick={() => {
                window.data.assignment.remove(
                  assignment.id,
                  `[${assignment.subject.name}] 删除作业 ${assignment.title}`,
                );
                props.popPage();
              }}
            >
