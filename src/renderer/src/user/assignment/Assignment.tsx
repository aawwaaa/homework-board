...
export const AssignmentPage: React.FC<{ props: UserPageProps }> = ({
  props,
}) => {
  ...
  tabs.push(
    ...(subjects.map((subject) => [subject.id, subject.name]) as [
      string,
      string,
    ][]),
  );
  ...
  useEffect(() => {
    return window.data.onChanged(async () => {
      ...
      setSubjects(
        (await window.data.subject.list()).filter((subject) =>
          isSubjectAccessible(props.identity, subject),
        ),
      );
    ...
    <div className="assignment-page">
      <div className="assignment-page__header">
        <Tab
          tabs={tabs}
          set={(id: string) =>
            setSubject(subjects.find((subject) => subject.id === id)!)
          }
          initial={subject?.id}
        />
        <button
      ...
      <div className="assignment-page__timeline">
        <TimelineView
          ...
          builder={(a, p) =>
            standardTimelineItemBuilder(a, p, () =>
              props.updatePage(
                "homeworkDetail",
                <AssignmentManage
                  key={Math.random()}
