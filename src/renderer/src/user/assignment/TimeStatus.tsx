...
export const TimeStatus: FC<{ assignment: AssignmentData }> = ({ assignment }) => {
    const [students, setStudents] = useState<Student[]>([]);
    const data = useMemo(() => {
        const data = assignment.submissions.filter(submission => submission.spent != null);
    ...
    return <>
        <div>
            ...
            <div>数据量: {data.dataAmount} / {assignment.submissions.length} ({Math.round(data.dataAmount / assignment.submissions.length * 100)}%)</div>
            ...
            <div className="data">
                {data.sorted.map(submission =>
                    <div key={submission.id} className="progress time-data">
                        <div className="bar" style={{ width: `${submission.spent! / data.max * 100}%` }} />
                        <span>{students.find(student => student.id === submission.student.id)?.name}</span>
