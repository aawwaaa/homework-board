...
export const CreateAssignment: React.FC<{ subject: Subject, props: UserPageProps }> = ({ subject, props }) => {
    ...
    const confirm = () => {
        window.data.assignment.create(assignment, `[${subject.name}] 布置作业 ${assignment.title}`);
        props.popPage();
    };

    return <div>
        <AssignmentEdit value={assignment} presets={subject.config.assignmentPresets} onChange={handleChange} />
