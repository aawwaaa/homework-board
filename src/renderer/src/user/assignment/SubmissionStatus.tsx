...
export const SubmissionStatus: FC<{ assignment: AssignmentData }> = ({
  assignment,
}) => {
  ...
  const submissionSet = useMemo(
    () =>
      new Set(
        assignment.submissions.map((submission) => submission.student.id),
      ),
    [assignment],
  );
    ...
    <>
      <div>
        <div className="progress">
          <div
            className="bar"
            style={{
              width: `${(assignment.submissions.length / assignment.totalRequiredSubmissions) * 100}%`,
            }}
          />
          <span>
            提交进度 {assignment.submissions.length} /{" "}
      ...
      <div>
        <div className="title" style={{ width: "100%" }}>
          ...
          <a
            style={{ float: "right", fontWeight: "normal", cursor: "pointer" }}
        ...
        <div className="unsubmitted">
          {(() => {
            const target = students.filter(
              (student) => inversed != !submissionSet.has(student.id),
            );
