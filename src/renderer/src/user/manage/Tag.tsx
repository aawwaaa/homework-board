...
const TagElement = ({
  ...
  tag: AssignmentTag;
  props: UserPageProps;
  onChange: (tag: AssignmentTag) => void;
}) => {
  const update =
    (key: keyof AssignmentTag) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="subject-row">
      ...
      />
      <button className="danger" onClick={() => window.data.tag.remove(tag.id)}>
        删除
      </button>
    </div>
  );
...
export const ManageTagPage = ({ props }: { props: UserPageProps }) => {
  ...
  const loadPreset = async (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    await Promise.all(
      (await window.data.tag.list()).map((tag) =>
        window.data.tag.remove(tag.id),
      ),
    );
  ...
  return (
    <div>
      ...
      <button className="outline" onClick={addTag}>
        添加
      </button>
