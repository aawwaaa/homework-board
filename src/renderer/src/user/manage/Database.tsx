            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.9rem" }}>
                ...
                <tbody>
                    {rows.length ? (
                        ...
                        <tr>
                            <td colSpan={columns.length} style={{ padding: "0.4rem", textAlign: "center", color: "var(--gray-tinted-6)" }}>
                                没有数据
                            </td>
                        </tr>
                    )}
...
export const ManageDatabasePage = () => {
    ...
    const [sqlResultRows, setSqlResultRows] = useState<Record<string, unknown>[] | null>(null);
    ...
    const loadTableData = useCallback(async (table: string) => {
        ...
        try {
            ...
            const columns = await window.data.database.all<TableColumnInfo>(`PRAGMA table_info(${identifier})`);
    ...
    const executeSql = useCallback(async () => {
        ...
        try {
            if (/^select/i.test(statement)) {
                const rows = await window.data.database.all<Record<string, unknown>>(statement);
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            <section style={{ padding: "0.75rem 0", borderBottom: "1px solid var(--gray-tinted-3)" }}>
                <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <h3 style={{ margin: 0 }}>数据表</h3>
                    <button className="flat" onClick={() => void loadTables()} disabled={tablesLoading}>
                        {tablesLoading ? "刷新中..." : "刷新"}
                    </button>
                </header>
                {tablesError ? <p style={{ color: "var(--color-danger, #b00020)" }}>{tablesError}</p> : null}
                <select
                    value={selectedTable}
            ...
            <section style={{ padding: "0.75rem 0", borderBottom: "1px solid var(--gray-tinted-3)" }}>
                <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                        ...
                        {selectedTable ? <small style={{ color: "var(--gray-tinted-6)" }}>显示前 {ROW_LIMIT} 行</small> : null}
                    </div>
                    <button className="flat" onClick={() => void loadTableData(selectedTable)} disabled={tableLoading}>
                        {tableLoading ? "加载中..." : "重新加载"}
                    </button>
                </header>
                {tableError ? <p style={{ color: "var(--color-danger, #b00020)" }}>{tableError}</p> : null}
                        ...
                        <ul style={{ margin: "0.5rem 0 0", paddingLeft: "1.2rem" }}>
                            {tableColumns.map((column) => (
                                <li key={column.cid}>
                                    ...
                                    {column.dflt_value !== null ? ` · 默认 ${column.dflt_value}` : ""}
                                </li>
                            ))}
                        </ul>
                    </div>
            ...
            <section style={{ padding: "0.75rem 0" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button className="primary" onClick={() => void executeSql()} disabled={sqlRunning}>
                        {sqlRunning ? "执行中..." : "执行"}
                    </button>
                    <button
                        className="flat"
                ...
                {sqlError ? <p style={{ color: "var(--color-danger, #b00020)" }}>{sqlError}</p> : null}
