  ...
  return (
    <div className="identity-row">
      ...
      <div className="roles">
        ...
        {role
          .split(",")
          .map((role) => subjects.find((a) => a.id == role))
          .filter(Boolean)
          .map((subject) => (
            <Badge
              key={subject!.id}
              data={subject!}
              onClick={() => removeRole(subject!.id)}
            />
          ))}
      ...
      <button
        className="danger"
        onClick={() => window.data.identity.remove(identity.id)}
      >
        删除
      </button>
    </div>
...
export const ManageIdentityPage = ({ props }: { props: UserPageProps }) => {
  ...
  const loadPreset = async (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    await Promise.all(
      preset.map((identity) => window.data.identity.add(identity)),
    );
  ...
  return (
    <div className="identity-manage">
      ...
      <button className="outline" onClick={addIdentity}>
        添加
      </button>
