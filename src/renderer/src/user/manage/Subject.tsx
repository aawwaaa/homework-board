...
const presetSubject: Record<
  "yu_wen" | "shu_xue" | "ying_yu" | "wu_li" | "hua_xue" | "sheng_wu",
  Subject
> = {
  yu_wen: {
    id: "1yu",
...
const presets = {
  语数英: [presetSubject.yu_wen, presetSubject.shu_xue, presetSubject.ying_yu],
  语数英物化生: [
...
const SubjectElement = ({
  ...
  const update =
    (key: keyof Subject) => (event: ChangeEvent<HTMLInputElement>) => {
    ...
    <div className="subject-row">
      ...
      <button
        ...
        onClick={() =>
          props.updatePage(
            "config",
            <SubjectConfigPage key={Math.random()} subject={subject} />,
          )
        }
      >
        配置
      ...
      <button
        className="danger"
        onClick={() => window.data.subject.remove(subject.id)}
      >
        删除
      </button>
    </div>
...
export const ManageSubjectPage = ({ props }: { props: UserPageProps }) => {
  ...
  const updateSubject = useCallback(
    async (next: Subject) => {
      ...
      setSubjects((current) => {
        const updated = current.map((subject) =>
          subject.id === next.id ? next : subject,
        );
        return updated;
      });
  ...
  const loadPreset = async (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    await Promise.all(
      (await window.data.subject.list()).map((subject) =>
        window.data.subject.remove(subject.id),
      ),
    );
    await Promise.all(
      preset.map((subject) => window.data.subject.add(subject)),
    );
  ...
  return (
    <div>
      ...
      <button className="outline" onClick={addSubject}>
        添加
      </button>
