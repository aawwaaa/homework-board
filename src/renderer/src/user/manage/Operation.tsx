...
const getFilterDate = (value: string) => (value ? new Date(`${value}T00:00:00`) : undefined);
...
export const ManageOperationPage = () => {
    ...
    const loadLatest = useCallback(() => requestLogs({ offset: 0, replace: true }), [requestLogs]);
        ...
        <div className="operation-panel">
            ...
            <div className="operation-controls">
                <div className="operation-control-group">
                    <input
                        ...
                        aria-label="按日期筛选"
                    />
                    <button type="button" className="flat" onClick={handleClearDate} disabled={!dateFilter}>
                        清除日期
                    </button>
                </div>
                <input
                ...
                />
                <button type="button" className="outline" onClick={loadLatest} disabled={loading}>
                    {loading ? "刷新中" : "刷新"}
                </button>
            </div>
            ...
            <div className="operation-list">
                ...
                {logs.map((log) => {
                    ...
                    return (
                        <div key={log.id} className={`operation-entry${log.reverted ? " operation-entry--reverted" : ""}`}>
                            <div className="operation-row">
                                ...
                                >
                                    <span className="operation-row-time">{formatTimestamp(log.created)}</span>
                                    <span className="operation-row-description">{log.description || "（无描述）"}</span>
                                </button>
                                <button
                                    type="button"
                            ...
                            {expandedRow && (
                                <div className="operation-details">
                                    ...
                                    {changes ? (
                                        ...
                                        <div className="operation-detail-data">没有附加变更数据。</div>
                                    )}
                                </div>
                            )}
                        </div>
