...
const getGroupLabel = (value: string) =>
  value?.trim().length ? value.trim() : GROUP_PLACEHOLDER;
...
const guessColumnIndex = (
  columns: string[],
  keywords: string[],
): string | null => {
  const normalized = columns.map((value) =>
    value.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]/g, ""),
  );
...
const StudentRow = ({
  ...
  const handleChange =
    (key: keyof Student) => (event: ChangeEvent<HTMLInputElement>) => {
...
const StudentImportPanel = ({
  ...
  const { columns, dataRows } = useMemo(() => {
    ...
    const normalizedHeader = header.map(
      (value, index) => value || `列 ${index + 1}`,
    );
    const filteredRows = rest.filter((row) =>
  ...
  useEffect(() => {
    ...
    setIdColumn((prev) =>
      prev && Number(prev) < columns.length
        ? prev
        : (guessColumnIndex(columns, ["id", "学号", "编号"]) ?? ""),
    );
    setGroupColumn((prev) =>
      prev && Number(prev) < columns.length
        ? prev
        : (guessColumnIndex(columns, ["group", "班", "班级", "组"]) ?? ""),
    );
    setNameColumn((prev) =>
      prev && Number(prev) < columns.length
        ? prev
        : (guessColumnIndex(columns, ["name", "姓名", "学生"]) ?? ""),
    );
  }, [columns]);
  ...
  const columnOptions = useMemo(
    () =>
      columns.map((title, index) => {
        const sample =
          dataRows.find((row) => row[index]?.trim().length)?.[index] ?? "";
        return {
          value: String(index),
...
export const ManageStudentPage = () => {
  ...
  const groupedStudents = useMemo(() => {
    ...
    return Array.from(groups.entries()).map(
      ([group, list]) => [group, [...list]] as const,
    );
  }, [students]);
  ...
  const handleImportResult = (summary: ImportSummary | null) => {
    ...
    if (summary) {
      ...
      announce(
        parts.length ? `${parts.join("，")} 名学生` : "没有可导入的数据",
      );
    }
  };
    ...
    <div className="student-manage">
      ...
      <div className="student-toolbar">
        <div className="student-actions">
          <button
            className="primary"
            onClick={() => setAddDrawerOpen((open) => !open)}
          >
            添加
          </button>
          <button
            className={clearConfirm ? "danger" : "outline"}
            disabled={!students.length || clearing}
            onClick={handleClear}
          >
