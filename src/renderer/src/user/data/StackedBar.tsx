...
export const StackedBar: React.FC<{
  ...
  const shouldMeasure =
    typeof minLabelWidthPx === "number" && minLabelWidthPx > 0;
  ...
  const segmentVisibility = segments.map((segment) => {
    if (!shouldMeasure || !isMeasuringReady || total === 0) {
      return {
        ...segment,
        showLabel: Boolean(segment.label) && !shouldMeasure,
      };
    }
    const ratio = total === 0 ? 0 : segment.flex / total;
    return {
      ...segment,
      showLabel: Boolean(segment.label) && width * ratio >= minLabelWidthPx!,
    };
  });

  const hiddenLegend = legendClassName
    ? segmentVisibility.filter(
        (segment) => !segment.showLabel && segment.legend != null,
      )
    : [];
