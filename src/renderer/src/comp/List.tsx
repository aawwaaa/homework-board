...
export const CompListConfig: FC<{
  config: ComponentListConfig;
  setConfig: (config: any) => void;
}> = ({ config, setConfig }) => {
  const { stateConfig, setStateConfig } =
    useComponentConfigState<ComponentListConfig>(
      defaultValue,
      config,
  ...
  return (
    <>
      ...
      <div className="comp-config-group">
        <h3>列表</h3>
        {helper.swipeInput("submittedDuration", "已提交区间", 1, "小时")}
        {helper.swipeInput("submittingDuration", "提交中区间", 1, "小时")}
        {helper.swipeInput("forward", "前向", 1, "天")}
      </div>
    </>
  );
};
...
export const compList: Component<ComponentListConfig> = {
  ...
  body: (config, openConfigWindow) => (
    <CompList config={config} openConfigWindow={openConfigWindow} />
  ),
  config: (config, setConfig) => (
    <CompListConfig config={config} setConfig={setConfig} />
  ),
};
